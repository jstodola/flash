...
AVR_TOOLS_PATH:=/home/honza/avr-gcc/usr/local/avr/bin/

# Install path of the arduino software. Requires a '/' at the end.
ARDUINO_PATH:=/home/honza/arduino/git/Arduino/
...
AVRDUDE_FLAGS = -V -F
AVRDUDE_FLAGS += -C $(ARDUINO_PATH)build/linux/dist/tools/avrdude.conf
AVRDUDE_FLAGS += -p $(MCU)
